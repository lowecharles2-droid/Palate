...
function parseSmartSearch(promptValue) {
  ...
  smartPromptText = prompt.toLowerCase();
  smartCategories = [];
  ...
  if (intent.maxDistance != null) {
    ...
    const selected = options.find(option => option >= intent.maxDistance) || 15;
  ...
  elements.smartExplanation.textContent = explanation.length
    ? `Searching for: ${explanation.join(' · ')}.`
    : 'Searching restaurant names, cuisines, dishes, and verified tags.';
}
...
function getFilteredRestaurants() {
  ...
  let filtered = restaurants.filter(restaurant => {
    ...
    return matchesDistance && matchesCategory && matchesCollection;
  });
...
function sortRestaurants(list, sort, hasSearch = false) {
  ...
  if (sort === 'authenticity') return copy.sort((a, b) => Number(b.authenticityScore) - Number(a.authenticityScore) || Number(b.searchScore || b.matchScore) - Number(a.searchScore || a.matchScore));
  if (sort === 'local') return copy.sort((a, b) => Number(b.localScore) - Number(a.localScore) || Number(b.searchScore || b.matchScore) - Number(a.searchScore || a.matchScore));
...
function getGroupLabel(restaurant) {
  const broadCategories = ['Chinese', 'Taiwanese', 'Indian', 'Japanese', 'Korean', 'Mexican', 'Thai', 'Vietnamese', 'Mediterranean', 'Middle Eastern', 'Italian', 'American', 'Greek', 'Hawaiian', 'Seafood', 'Healthy'];
