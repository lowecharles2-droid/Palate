...
The automated suite validates all supported cuisine/search rules, chain-versus-specialist ranking, restaurant data integrity, cache protection, and the upgraded MVP interface. A separate browser audit completed 107 end-to-end interaction checks; see `TEST_REPORT.md`.

## Data and MVP limitations

Distances outside UCSD are approximate and use neighborhood centers. Exact navigation opens through each restaurant’s directions link. Current-location detection requires HTTPS and browser permission. Reports and analytics are stored in the current browser for MVP testing; a production version should connect them to a persistent database and analytics provider.
