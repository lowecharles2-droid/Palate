...
const STORAGE_KEYS = {
  ...
  onboarding: 'palate_onboarding_complete_v2',
...
let groupMode = { active: false, cuisines: [], dietary: [], budget: 'any', distance: 999 };
...
function renderActiveFilters() {
  ...
  elements.activeFilters.querySelectorAll('[data-filter-type]').forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (type === 'group') groupMode = { active: false, cuisines: [], dietary: [], budget: 'any', distance: 999 };
...
function renderRestaurants() {
  ...
  if (!filtered.length) {
    ...
    document.querySelector('#relaxFilters')?.addEventListener('click', () => {
      selectedCategory = 'All'; selectedCollection = 'All'; elements.distanceLimit.value = '999'; elements.priceLimit.value = 'any'; groupMode.active = false; renderRestaurants();
      track('relax_filters');
    });
...
async function shareRestaurant(restaurant) {
  ...
  try {
    if (navigator.share) await navigator.share({ title: restaurant.name, text: shareText });
    else { await navigator.clipboard.writeText(shareText); showToast('Restaurant details copied'); }
...
function renderSavedModal() {
  ...
  if (!savedRestaurants.length) {
    ...
    document.querySelector('#savedBrowse')?.addEventListener('click', () => { elements.savedModal.close(); document.querySelector('#discover').scrollIntoView({ behavior: 'smooth' }); });
    return;
  }
  ...
  elements.savedContent.querySelectorAll('.saved-view').forEach(button => button.addEventListener('click', () => { elements.savedModal.close(); openDetails(button.dataset.id); }));
...
function resetFilters() {
  ...
  persistState(); buildSliders(); loadRestaurants(); showToast('Filters reset'); track('reset_filters');
}
...
function updateLocationUI() {
  ...
  elements.locationSummary.textContent = `Distances are estimated from ${locationState.label}.`;
  elements.locationNav.textContent = locationState.label;
}

function openLocation() {
  updateLocationUI();
  elements.customLocationWrap.hidden = elements.modalLocationSelect.value !== 'custom';
  elements.locationModal.showModal();
}

function setLocationFromChoice(choice, customText = '') {
  if (choice === 'current') {
    if (!navigator.geolocation) return showToast('Location access is not available in this browser');
    ...
    navigator.geolocation.getCurrentPosition(position => {
      ...
      persistState(); updateLocationUI(); renderRestaurants(); elements.locationModal.close(); showToast('Location updated'); track('location_change', { source: 'current' });
    }, () => showToast('Location permission was not granted'), { enableHighAccuracy: false, timeout: 8000 });
    return;
  }
  if (choice === 'custom') {
  ...
  persistState(); updateLocationUI(); renderRestaurants(); elements.locationModal.close(); showToast(`Using ${locationState.label}`); track('location_change', { location: locationState.key });
}
...
function submitOnboarding(event) {
  ...
  persistState(); buildSliders(); elements.onboardingModal.close(); loadRestaurants(); showToast('Your recommendations are ready'); track('onboarding_complete', userPrefs);
}
...
function clearGroupMode() {
  ...
  elements.groupBudget.value = 'any'; elements.groupDistance.value = '5';
  elements.groupModal.close(); renderRestaurants(); showToast('Group mode cleared');
}

function initializeEvents() {
  elements.keywordSearch.addEventListener('input', renderRestaurants);
  elements.distanceLimit.addEventListener('change', () => { renderRestaurants(); track('distance_filter', { value: elements.distanceLimit.value }); });
  elements.priceLimit.addEventListener('change', () => { renderRestaurants(); track('price_filter', { value: elements.priceLimit.value }); });
  elements.sortBy.addEventListener('change', () => { renderRestaurants(); track('sort', { value: elements.sortBy.value }); });
  elements.resetFilters.addEventListener('click', resetFilters);
  elements.runSmartSearch.addEventListener('click', () => parseSmartSearch(elements.smartPrompt.value));
  elements.smartPrompt.addEventListener('keydown', event => { if (event.key === 'Enter') parseSmartSearch(elements.smartPrompt.value); });
  elements.heroSearchForm.addEventListener('submit', event => { event.preventDefault(); elements.smartPrompt.value = elements.heroPrompt.value; parseSmartSearch(elements.heroPrompt.value); document.querySelector('#discover').scrollIntoView({ behavior: 'smooth', block: 'start' }); });
  document.querySelectorAll('[data-prompt]').forEach(button => button.addEventListener('click', () => { const prompt = button.dataset.prompt; elements.heroPrompt.value = prompt; elements.smartPrompt.value = prompt; parseSmartSearch(prompt); document.querySelector('#discover').scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
  ...
  elements.savedNav.addEventListener('click', () => { renderSavedModal(); elements.savedModal.showModal(); track('open_saved'); });
  elements.locationNav.addEventListener('click', openLocation);
  ...
  elements.locationSelect.addEventListener('change', () => { if (elements.locationSelect.value === 'custom') openLocation(); });
  elements.modalLocationSelect.addEventListener('change', () => { elements.customLocationWrap.hidden = elements.modalLocationSelect.value !== 'custom'; });
  elements.locationForm.addEventListener('submit', event => { event.preventDefault(); setLocationFromChoice(elements.modalLocationSelect.value, elements.customLocationInput.value); });
  elements.closeLocation.addEventListener('click', () => elements.locationModal.close());

  elements.groupNav.addEventListener('click', () => elements.groupModal.showModal());
  elements.groupForm.addEventListener('submit', submitGroupMode);
  elements.clearGroupMode.addEventListener('click', clearGroupMode);
  elements.closeGroup.addEventListener('click', () => elements.groupModal.close());
  ...
  elements.onboardingLater.addEventListener('click', skipOnboarding);
  elements.reportForm.addEventListener('submit', submitReport);
  elements.closeReport.addEventListener('click', () => elements.reportModal.close());

  [elements.detailModal, elements.savedModal, elements.locationModal, elements.groupModal, elements.reportModal].forEach(modal => modal.addEventListener('click', event => { if (event.target === modal) modal.close(); }));
}
...
loadRestaurants();
track('app_open');
if (storageGet(STORAGE_KEYS.onboarding) !== 'true') setTimeout(() => elements.onboardingModal.showModal(), 500);
